...
import { addFilters } from "./filterSlice";
...
export const Myrequest = () => {
  ...
  useEffect(() => {
    setTableHeight(height - ref?.current?.offsetHeight - 88);
  }, [height]);

  // const saveFilteredResultsToLocalStorage = (filteredResults) => {
  //   try {
  //     const serializedData = JSON.stringify(filteredResults);
  //     localStorage.setItem('filteredResults', serializedData);
  //   } catch (error) {
  //     console.error('Error while saving filtered results to localStorage:', error);
  //   }
  // };
  ...
  const getAllTickets = (
    ...
    getTickets(
      ...
      .then((res) => {
        ...
        setIsLoading(false);
        // saveFilteredResultsToLocalStorage(res?.data?.Items);
        dispatch(addFilters(res?.data?.Items))
      })
      .catch((err) => {
        setIsLoading(false);
        Swal.fire({
  ...
  /**UseSelector for getting the filtered ticketdetails from the components */
  const filterRequest = useSelector(state => state.filterRequest);
